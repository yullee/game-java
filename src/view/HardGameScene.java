...
import entity.HardPlayer;
import frame.Frame;
import item.SpeedItem;
import item.YellowFlower;
import view.Sprite;

public class HardGameScene extends Scene {
...
//    public Sprite hardAnimation;
	...
	public HardGameScene(GameController controller) {
		...
		healthScore = new SimpleStringProperty(Integer.toString(hardPlayer.getHealth())); 
...
//			System.out.println("Enemy sound!");
	...
	public void createPlayer() {
		hardPlayer = new HardPlayer((fWidth / 2) , (fHeight / 2) );
		System.out.println("Hard player is created.");
		pane.getChildren().addAll(hardPlayer);
	}
...
//			if(player.getXOrigin() < 0 || player.getYOrigin() < 0 || player.getXOrigin() > pane.getPrefWidth() || player.getYOrigin() > pane.getPrefHeight()) {
			...
			//	    	 if (isPressed(KeyCode.UP)) {
				hardPlayer.animation.play();		
				hardPlayer.animation.setOffsetY(35);
				hardPlayer.animation.setOffsetX(143);
				hardPlayer.moveY(-4);
			} else if(e.getCode() == KeyCode.S) {
				hardPlayer.animation.play();
				hardPlayer.animation.setOffsetY(0);	
				hardPlayer.animation.setOffsetX(146);
				hardPlayer.moveY(4);
			} else if(e.getCode() == KeyCode.D) {
				hardPlayer.animation.play();
				hardPlayer.animation.setOffsetX(200);
				hardPlayer.animation.setOffsetY(0);
				hardPlayer.moveX(4);
			} else if(e.getCode() == KeyCode.A) {
				hardPlayer.animation.play();
				hardPlayer.animation.setOffsetY(0);
				hardPlayer.animation.setOffsetX(87);
				hardPlayer.moveX(-4);
			} else {
				hardPlayer.animation.stop(); // Else stop the animation
			}
		});
	}
	...
	public void createComponents(){		
		...
		pane.setStyle("-fx-background-image: url(\"images/desert_bg.jpg\");");
	...
	public void badCollision(Entity players, ImageView b) {
	    if (players.getBoundsInParent().intersects(b.getBoundsInLocal())) {
	        ...
	        hardPlayer.setHealth(hardPlayer.getHealth() - 30);
	        musicPlayer();
	        if(hardPlayer.getHealth() <= 0) { // If the player's health is 0, the game ends 
		...
		AnimationTimer animation = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				for(Enemy c: poisonList) {
					moveCharacter(c);
					badCollision(hardPlayer, c); // detect a collision
				}
				...
				for(Enemy c: speedItemList) {
					moveCharacters(c);
					goodCollision(hardPlayer, c); // detect a collision
				}
